...
@Autowired
	...
	Session session=sessionFactory.getCurrentSession();
		SQLQuery sqlQuery=session.createSQLQuery("select * from user_table where email in (select email from user_table where email!=? minus (select fromId from friend_table1 where toId=? union select toId from friend_table1 where fromId=?))");
	...
	public List<User> listOfFriends(String email) {
		...
		session.createSQLQuery("select * from user_table where email in"
				+ "(select fromId from friend_table1 where toId=? and status='A' "
				+ " union "
				+ "select toId from friend_table1 where fromId=? and status='A')");
		query.setString(0, email);
